    ...
    with open(proof_file, "r") as f:
        ...
        for index, line in enumerate(proofreader):
            if index == 0:
                ...
                new_row.append(TreeNode(bytearray.fromhex(line[0]), int(line[1]), index+1))
                assert len(line) == 2
...
def validate_liabilities(block_height, tree, account, nonce, account_nonce, args):
    ...
    for leaf_index in range(len(tree[0])):
        ...
        if leaf_hash(tree[0][leaf_index][1], nonce_bytes, leaf_index) != tree[0][leaf_index].hash:
            continue

        print('Hash match for leaf {} claims {:,} sats'.format(leaf_index, tree[0][leaf_index].sats))
        ...
        for height in range(len(tree)-1):
            ...
            if expected_parent.hash != parent_node.hash:
                raise Exception("Tree node hash computation mismatch: {} vs {}".format(expected_parent, parent_node))
            if expected_parent.sats != parent_node.sats:
                raise Exception("Tree node sats computation mismatch: {} vs {}".format(expected_parent, parent_node))
        ...
        if args.print_proof_csv:
            print("{},{},{},{}".format(
                ...
                block_height,
                proof_nodes[0].sats,
                ','.join("{},{}".format(n.hash.hex(), n.sats) for n in proof_siblings)
            ))
        elif args.print_tree:
            for p,s in itertools.zip_longest(proof_nodes, proof_siblings):
                print('  {} {:26,} {:10} {:10}'.format(p.hash.hex(), p.sats, p.lineno, s.lineno if s else ""))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--print-proof-csv',
        action='store_true',
        help='print proof matches as comma separated list of nodes', default=False)
    parser.add_argument(
        '--print-tree',
        action='store_true',
...
if __name__ == '__main__':
    ...
    validate_liabilities(block_height, tree, args.account, args.nonce, args.account_nonce, args)
